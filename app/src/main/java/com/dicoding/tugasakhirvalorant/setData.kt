package com.dicoding.tugasakhirvalorant



object setData {

    val agentName = arrayOf(
        "Astra",
        "Breach",
        "Chamber",
        "Jett",
        "KillJoy",
        "Neon",
        "Raze",
        "Reyna",
        "Viper",
        "Yoru")

    val agentPhoto = intArrayOf(
        R.drawable.astra,
        R.drawable.breach,
        R.drawable.chamber,
        R.drawable.jett,
        R.drawable.killjoy,
        R.drawable.neon,
        R.drawable.raze,
        R.drawable.reyna,
        R.drawable.viper,
        R.drawable.yoru,)

    val agentRole = arrayOf(
        "Controller",
                "Initiators",
                "Sentinels",
                "Duelist",
                "Sentinels",
                "Duelist",
                "Duelist",
                "Duelist",
                "Controller",
                "Duelist",)

    val agentRealName = arrayOf(
        "Efia Danso",
               "Erik Torsten",
                "Vincent Fabron",
               "Sunwoo Han",
                "Klara Bohringer",
                "Nicole Dimaapi Valdez",
              "Tayane Alves",
             "Zyanya Mondragón",
              "Sabine Calla",
            "Ryo Kiritani",)
    val agentGender = arrayOf(
       "Female",
           "Male",
               "Male",
               "Female",
         "Female",
          "Female",
           "Female",
             "Female",
              "Female",
            "Male",)
    val agentNegara = arrayOf(
        "Ghana",
                "Swedia",
         "Perancis",
              "South Korea",
              "Jerman",
                "Filipina",
                "Brazil",
            "Mexico",
                "Amerika Serikat",
            "Jepang",)



    val deskripsi = arrayOf(
        "Digambarkan sebagai seorang agent berkebangsaan Ghana, Astra bisa mengubah arena pertarungan sesuai dengan keinginannya. Dengan kemampuan astralnya dia bisa menjadi controller yang sangat bisa diandalkan.\n" +
                "\n" +
                "Jurus-jurus yang dimiliki Astra: \n" +
                "\n" +
                "Nova pulse: Jurus ini memungkinkan Astra untuk menempatkan bintang dalam bentuk astral untuk meledakkan sebuah Nova Pulse. Dengan begitu semua musuh yang terkena ledakkan akan terdampak dan membuat semua musuh seolah terkena gegar otak.\n" +
                "\n" +
                "Nebula: Kemampuan ini bisa untuk menghambat penglihatan musuh. \n" +
                "\n" +
                "Gravity well: Jurus ini memungkinkan Astra untuk menghasilkan sebuah sumur atau area yang memiliki gravitasi. Begitu musuh tersedot ke dalamnya sumu ratau area ini pun akan meledak. \n" +
                "\n" +
                "Astral form/Cosmic divide: Dengan skill ini, Astra bisa membuat sebuah dinding yang kebal terhadap peluru dan membuat suasana sunyi.",

        "Karakter initiator di Valorant yang pertama adalah Breach. Bionik asala Swedia ini punya tembakan yang sangat kuat dan bisa membuka jalan saat ingin memulai pertarungan. Apalagi damage yang dihasilkannya tidak main-main, sakit sekali.\n" +
                "\n" +
                "Jurus-jurus yang dimiliki Breach:\n" +
                "\n" +
                "Flashpoint: Breach bisa menembakan senjatanya untuk memberikan efek buta pada musuh. Skill ini pun bisa menembus tembok.\n" +
                "\n" +
                "Fault line: Breach bisa menghasilkan sebuah ledakan seismik yang akan mambuat musuh pusing bila terkenanya.\n" +
                "\n" +
                "Aftershock: Dengan skill ini Breach bisa menghasilkan sebuah fusion charge. Jika musuh berada di dalam zona tersebut akan terkena damage yang besar.\n" +
                "\n" +
                "Rolling thunder: Skill ultimate ini menghasilkan sebuah seismic charge yang akan membuat musuh pusing dan terlempar saat berada di dalam areanya.",

        "Berpakaian rapi dan bersenjata lengkap, desainer senjata dari Prancis, Chamber, menghalau penyerang dengan presisi yang mematikan. Dia memanfaatkan persenjataan kustomnya untuk mempertahankan barisan, menembak musuh dari jauh, dengan kontingensi yang tersusun untuk tiap rencana\n" +
                "\n" +
                "Jurus Jurus yg DImiliki Chamber\n" +
                "Trademark\n" +
                "Skill ini akan membuatmu bisa menempatkan jebakan yang dapat melakukan scanning musuh. Ketika musuh masuk ke dalam jangkauannya, jebakan tersebut akan melakukan hitung mundur dan kemudian membuat area di sekitarnya menjadi tidak stabil.\n" +
                "Agent lawan yang masuk ke dalam area jebakan ini akan melambat gerakannya sekaligus terperangkap di dalamnya.\n" +
                "Headhunter\n" +
                "Skill ini perlu kamu aktifkan untuk mengeluarkan heavy pistol yang menjadi salah satu kekuatan utama dari Chamber. ALT FIRE dengan pistol yang dilengkapi untuk mengarahkan pandangan ke bawah.\n" +
                " Rendezvous\n" +
                "Skill ini memungkinkan Chamber melakukan teleportasi melalui penempatan dua anchor teleportasi. Kamu bisa berpindah dari satu anchor ke anchor lainnya. Kemudian, anchor dapat diambil kembali untuk diletakkan di lokasi yang berbeda.\n" +
                " Tour de Force\n" +
                "Skill ini akan membuat Chamber mengeluarkan senjata sniper khusus yang kuat dan mampu membunuh musuh dengan serangan langsung ber-damage besar. Jika tembakanmu berhasil membunuh musuh, maka skill ini akan menciptakan medan yang akan memperlambat pemain yang terperangkap di dalamnya.\n",
        "Hero kedua dalam daftar agent Valorant dengan peran duelist adalah Jett. Berasal dari Korea Selatan, karakter ini hadir dengan elemen angin dan pisau sebagai senjata pamungkasnya. Kemampuannya untuk bergerak dengan cepat membuatnya jadi pembunuh yang andal.\n" +
                "\n" +
                "Jurus-jurus yang dimiliki Jett:\n" +
                "\n" +
                "Updraft: Secara instan akan membuat Jett naik ke ketinggian.\n" +
                "\n" +
                "Tailwind: Jurus ini akan mempercepat gerak Jett ke arah yang dipilih. Jika dalam keadaan diam, ia akan maju ke depan secara cepat.\n" +
                "\n" +
                "Cloudburst: Dengan jurus ini Jett bisa menghalangi penglihatan musuh dengan sebuah bola angin besar. \n" +
                "\n" +
                "Blade storm: Seperti yang telah dijelaskan sebelumnya, Jett punya senjata pisau yang akan muncul pada skill ultimate-nya. Jurus ini bisa mematikan musuh dengan sangat cepat.\n",
        "    Karakter ini digambarkan sebagai seorang yang jenius dan berasal dari Jerman. Ia bisa berperang dengan bermacam barang ciptaannya. Agent yang ini memiliki visual seorang wanita dengan tampilan geek dan memiliki jaket kuning yang cerah.\n" +
                "\n" +
                "Jurus-jurus yang dimiliki Killjoy:\n" +
                "\n" +
                "Alarmbot: Killjoy bisa menempatkan sebuah robot yang bisa menembak musuh bila dalam jangkauan. Saat menembak dan musuhnya mendekat, robot tersebut akan meledak.\n" +
                "\n" +
                "Turret: Skill ini memungkinkan Killjoy untuk membuat sebuah menara kecil yang bisa menembaki musuh dengan jangkauan sudut 180 derajat.\n" +
                "\n" +
                "Nanoswarm: Killjoy akan melemparkan sebuah granat Nanoswarm. Bila musuh terkena atau dalam jangkauan zonanya bisa dipastikan akan mati.\n" +
                "\n" +
                "Lockdown: Jurus ultimate dari Killjoy ini bisa menempatkan semacam alat lockdown. Begitu ditanamkan maka alat ini bisa menangkap semua musuh yang ada di dalam jangkauan. Akan tetapi, alat ini bisa dihancurkan oleh musuh.\n",
        "Agen dari Filipina, Neon, melesat maju dengan kecepatan yang mencengangkan, menyemburkan pancaran bioelektrik secepat tubuhnya menghasilkannya. Dia berlari untuk mengejutkan musuh, lalu menyerang mereka lebih cepat dari kilat.\n" +
                "        Jurus-jurus yang dimiliki Neon:\n" +
                "\n" +
                "        Fast Lane : menembakkan jalur energi melalui kedua tangannya dan terciptanya dua tembok yang terbuat dari listrik statis berwarna biru untuk menutup pandangan lawan, mirip seperti milik agent Phoenix.\n" +
                "\n" +
                "        Relay Bolt : melemparkan 2 bola energi dalam satu lemparan dan dapat memantulkan bola energi terakhir, yang menyebabkan ledakan berupa stun, durasinya mirip seperti Fault Line milik agent Breach.\n" +
                "\n" +
                "        High Gear : memberikan agent Neon sebuah kekuatan untuk meningkatkan kecepatan pada dirinya dan bisa sliding saat menggunakan skill tersebut.\n" +
                "\n" +
                "        Overdrive :melancarkan kekuatan penuh dan kecepatan dari agent Neon dalam waktu singkat, api untuk menyalurkan daya ke sinar pencahayaan yang mematikan dengan akurasi gerakan tinggi.\n",
        "Karakter di Valorant yang ini dikisahkan sebagai gadis yang menyukai ledakan dan berasal dari Brasil. Mungkin dia adalah agent yang sangat mengandalkan jurusnya jika dibandingkan dengan duelist lainnya. Akan tetapi, Raze bisa diandalkan untuk bertarung dalam jarak yang dekat.\n" +
                "\n" +
                "Jurus-jurus yang dimiliki Raze:\n" +
                "\n" +
                "Blast pack: Bisa melemparkan bom yang menempel ke mana saja. Raze tidak akan terkena damage bila bom ini meledak di dekatnya. Namun, jika karena ledakan ia jatuh, HP yang ia punya akan berkurang.\n" +
                "\n" +
                "Paint shells: Raze bisa melepaskan sebuah cluster granade. Granat ini akan melukai siapapun saat meledak.\n" +
                "\n" +
                "Boom bot: Raze akan melepaskan sebuah robot yang memiliki daya ledak. Robot ini bisa melihat dan mengunci musuh. Damage yang dihasilkan lumayan besar.\n" +
                "\n" +
                "Showstopper: Jurus ultimate ini memungkinkan Raze untuk melontarkan sebuah roket. Siapa pun yang terkena akan terluka parah.\n",
        "Agent yang satu ini dikisahkan berasal dari Meksiko. Kemampuannya bertarung jarak dekat membuatnya sangat dominan. Oleh karena itu, dalam pertarungan banyak tim yang mengandalkan Reyna sebagai pembunuh yang sangat ditakuti.\n" +
                "\n" +
                "Jurus-jurus yang dimiliki Reyna:\n" +
                "\n" +
                "Devour: Reyna bisa menyembuhkan dirinya setelah membunuh musuhnya dengan mengandalkan Soul Orb yang muncul.\n" +
                "\n" +
                "Dismiss: Saat mengonsumsi Soul Orb, Reyna bisa menjadi tidak terlihat dalam durasi yang singkat. Jurus ini tetap berjalan meski Empress juga diaktifkan.\n" +
                "\n" +
                "Leer: Jurus ini bisa menghadirkan mata berbentuk orbs ungu yang bisa dihancurkan. Namun, saat dilepaskan musuh akan mengalami gangguan penglihatan dalam waktu tertentu.\n" +
                "\n" +
                "Empress: Skill ultimat ini akan meningkatkan kemampuan menembak, mengisi ulang peluru, hingga mengganti senjata. Bila membunuh dengan jurus ini, kamu bisa menggunakan Empress dengan durasi lebih panjang.\n",
        "Ahli kimia dari Amerika Serikat ini bisa menyebarkan racun dan melumpuhkan penglihatan musuh. Selain itu, wanita yang identik dengan warna hijau ini pun bisa menjebak musuh dalam racunnya.\n" +
                "\n" +
                "Jurus-jurus yang dimiliki Viper: \n" +
                "\n" +
                "Poison cloud: Viper bisa melemparkan sebuah tabung berisi gas yang bisa menghalangi vision musuh. Menariknya, bila tabung tersebut diambil kamu bisa menggunakannya lagi.\n" +
                "\n" +
                "Toxic screen: Dengan jurus ini Viper bisa menghadirkan semacam tembok untuk menghalangi pandangan musuh. Sama seperti Poison cloud, skill ini pun bisa dipakai berulang kali.\n" +
                "\n" +
                "Snake bite: Viper bisa melemparkan cairan kimisa yang akan pecah dan bisa memperlambat pergerakan musuh.\n" +
                "\n" +
                "Viper’s pit: Viper bisa menghasilkan semacam kubah dengan racunnya dan jika musuh berada di dalamnya akan berkurang HP-nya. Oh iya, jurus ini juga berguna untuk menghalangi penglihatan musuh.\n",
        "        Yoru merupakan karakter yang digambarkan sebagai orang Jepang. Ia bisa menjadi pembunuh yang bisa membuat musuh tertipu dan masuk ke dalam garis belakang musuh tanpa terlihat. Agent ini sangat cocok untuk dijadikan rusher yang mengejar backline.\n" +
                "\n" +
                "Jurus-jurus yang dimiliki oleh Yoru:\n" +
                "\n" +
                "Blindside: Jurus ini bisa dipakai oleh Yoru untuk membutakan musuh dalam waktu tertentu dan bisa memantul pada permukaan yang keras.\n" +
                "\n" +
                "Gatecrash: Dengan jurus ini, Yoru bisa melaju dengan cepat begitu tether yang ia lepaskan sampai pada tempatnya. Semacam teleport menuju tether tersebut.\n" +
                "\n" +
                "Fakeout: Seperti namanya, skill ini bisa menipu musuh dengan langkah kaki yang palsu.\n" +
                "\n" +
                "Dimensional drift: Adanya jurus ultimate ini akan membuat Yoru menyusup ke area belakang lawan tanpa terlihat. Skill ini tidak bisa terpengaruh atau terlihat oleh musuh.\n",

        )

        val listData : ArrayList<AgentData>
        get() {
            val list = arrayListOf<AgentData>()
            for (position in agentName.indices) {
                val agent = AgentData()
                agent.agentName= agentName[position]
                agent.agentPhoto= agentPhoto[position]
                agent.agentRole= agentRole[position]
                agent.agentGender= agentGender[position]
                agent.agentRealname= agentRealName[position]
                agent.agentNegara= agentNegara[position]
                agent.agentDeskripsi = deskripsi[position]
                list.add(agent)
            }
            return list
        }

}